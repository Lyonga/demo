  ...
  "inputs": {
    ...
    "Parameters": {
      "commands": [
        ...
        "if (Test-Path 'C:\\Scripts\\CROWDSTRIKE.EXE') { Remove-Item -Path 'C:\\Scripts\\CROWDSTRIKE.EXE' -Force }",
        "Write-Host 'Downloading CrowdStrike installer...'",
        "Read-S3Object -BucketName '{{ S3BucketName }}' -Key 'agents/CROWDSTRIKE.EXE' -File 'C:\\Scripts\\CROWDSTRIKE.EXE'",
        "Write-Host 'Installing CrowdStrike...'",
        "Start-Process -FilePath 'C:\\Scripts\\CROWDSTRIKE.EXE' -ArgumentList '/install', '/quiet', '/norestart', 'CID={{ CrowdStrikeCID }}' -Wait"
      ]
    }
  }
}
