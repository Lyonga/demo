...
from datetime import datetime, timedelta
from botocore.exceptions import ClientError
from pprint import pprint  #  <-- For pretty-printing dictionaries in logs
...
def lambda_handler(event=None, context=None):
    ...
    mainCostDict = ce_get_costinfo_per_account(accountDict)
    print("\n[DEBUG] mainCostDict:")
    pprint(mainCostDict)  # <--- Pretty-print the output

    # 2) Re-sort the mainCostDict by month
    mainMonthlyDict = process_costchanges_per_month(mainCostDict)
    print("\n[DEBUG] mainMonthlyDict:")
    pprint(mainMonthlyDict)

    # 3) Create a "display" dictionary that includes 'Others' + 'monthTotal'
    mainDisplayDict = process_costchanges_for_display(mainMonthlyDict)
    print("\n[DEBUG] mainDisplayDict:")
    pprint(mainDisplayDict)

    # 4) Optionally compute monthly % changes
    finalDisplayDict = process_percentchanges_per_month(mainDisplayDict)
    print("\n[DEBUG] finalDisplayDict (with % changes):")
    pprint(finalDisplayDict)

    # 5) Generate the summary HTML
    summary_html = create_report_html(finalDisplayDict, BODY_HTML)
    print("\n[DEBUG] summary_html (first 500 chars):")
    print(summary_html[:500], "...\n")  # Just print a snippet to avoid massive logs

    # 6) Optional: Per-service breakdown
    account_numbers = list(accountDict.keys())  # or get_linked_accounts(...)
    cost_data_Dict = get_cost_data(account_numbers)
    print("\n[DEBUG] cost_data_Dict:")
    pprint(cost_data_Dict)

    display_cost_data_Dict = restructure_cost_data(cost_data_Dict, account_numbers)
    print("\n[DEBUG] display_cost_data_Dict (per-service):")
    pprint(display_cost_data_Dict)

    breakdown_html = generate_html_table(cost_data_Dict, display_cost_data_Dict)
    print("\n[DEBUG] breakdown_html (first 500 chars):")
    print(breakdown_html[:500], "...\n")

    combined_html = summary_html + '<br><br>' + breakdown_html

    # 7) Send the email
    print("\n[DEBUG] Final combined HTML (first 500 chars):")
